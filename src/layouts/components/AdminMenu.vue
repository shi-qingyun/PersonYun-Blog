<template>
  <div
    class="meun shadow-md fixed bg-light-50 transition-all duration-300"
    :style="{ width: $store.state.menuWidth }"
  >
    <div class="flex items-center justify-center h-[64px]">
      <img
        v-if="$store.state.menuWidth == '250px'"
        src="@/assets/svg/shandian.svg"
        class="h-[45px] mr-4"
      />
      <p class="text-white text-lg">PersonYun博客管理</p>
    </div>
    <el-menu
      :collapse="isCollapse"
...
.el-menu-item.is-active {
  background-color: #477cf8 !important;
  color: #fff;
}
</style>