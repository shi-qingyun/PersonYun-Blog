<template>
  <Header @FatherClick="throttledFatherClicks"></Header>
    ...
    <div class="grid grid-cols-4">
      ...
      <div class="col-span-4 px-3 md:col-span-3 sm:col-span-4">
        ...
        <nav aria-label="Page navigation example" v-if="total > 0">
          <ul
            class="flex items-center justify-center mt-10 mb-10 -space-x-px h-10 text-base"
          >
            <li>
              <a
                v-if="current > 1"
...
import { throttle } from "@/untiles/throttle.js";
...
const FatherClick = (value) => {
  if (value === "") {
    getArticles();
  }
  const a = articles.value.filter((item) => {
    return item.title.indexOf(value) !== -1;
  });

  articles.value = a;
};
const throttledFatherClicks = (value) => {
  throttle(FatherClick(value), 3000);
};
// 获取分类
